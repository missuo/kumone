...
struct AutomaticCacheLimitTests {
    ...
    @Test func neverReachesIntoTheReservedSpace() {
        #expect(AutomaticCacheLimit.megabytes(freeBytes: 2_500_000_000, cacheBytes: 0) == 1_500)
        #expect(AutomaticCacheLimit.megabytes(freeBytes: 800_000_000, cacheBytes: 0) == 0)
        #expect(AutomaticCacheLimit.megabytes(freeBytes: -1, cacheBytes: -1) == 0)
    }
}
